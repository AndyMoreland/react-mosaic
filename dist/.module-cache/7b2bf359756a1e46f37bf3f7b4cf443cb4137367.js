...
var Chunk = React.createClass({displayName: 'Chunk',
    ...
    style:function(){
        var posX = this.state.edgePx * this.state.homePoint[0],
            posY = this.state.edgePx * this.state.homePoint[1];

        if (this.state.image.orientation == 'vertical' && this.state.image.topOffset) 
            posY += this.state.image.topOffset;
        else if (this.state.image.orientation == 'horizontal' && this.state.image.leftOffset) 
            posX += this.state.image.leftOffset;
