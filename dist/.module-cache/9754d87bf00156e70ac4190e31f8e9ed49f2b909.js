    ...
    Header = React.createClass({displayName: 'Header',
        ...
        render : function(){
            var gameIcon = (this.state.isGame) ? 'av-replay' : 'av-play-arrow',
                dialogActions = [{text:'CANCEL'}];

            return (React.DOM.div({className: "toolbar"}, 
                Dialog({ref: "matrixDialog", title: "Set matrix", actions: dialogActions}, 
                    PaperButton({type: "DEFAULT", onClick: this._setMatrix33, label: "3X3"}), 
                    PaperButton({type: "DEFAULT", onClick: this._setMatrix44, label: "4X4"}), 
                    PaperButton({type: "DEFAULT", onClick: this._setMatrix55, label: "5X5"})
                ), 
