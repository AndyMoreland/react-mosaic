...
var Chunk = React.createClass({displayName: 'Chunk',
    propTypes: 
    {
        ...
        hole: React.PropTypes.arrayOf(React.PropTypes.number),
    ...
    render:function()
    {
        ...
        return (React.DOM.div({className: classes, style: this.style(), onMouseDown: this._onMouseDown, onMouseMove: this._onMouseMove, onMouseUp: this._onMouseUp}, 
    ...
    _onPaneChange: function()
    {
        var chunk = PaneStore.getChunk(this.state.homePoint),
            crawlDist = chunk.isCrawable ? _getCrawlDist(chunk.point) : null;
        if (chunk) {
            this.setState({
                point: chunk.point,
                isCrawable: chunk.isCrawable,
